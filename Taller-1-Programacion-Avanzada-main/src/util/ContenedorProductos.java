...
public class ContenedorProductos {
    ...
    public int obtenerProductoPosicion(Producto producto1){
        for (int i = 0; i < this.cantActual; i++) {
            if (this.inventario[i] == producto1){
                return i;
            }
        }
        return -1;
    ...
    public void reemplazarProducto(Producto producto){
        for (int i = 0; i < this.cantActual; i++) {
            if (producto.getNombreProducto().equalsIgnoreCase(this.inventario[i].getNombreProducto())){
    ...
    /**
     * metodo que desplegara la orden que posee el cliente en el rastaurante
     */
    public void desplegarProductosCliente(){
        //Si no hay productos, se despliega esto
        if (this.cantActual == 0){
            System.out.println("No hay Productos registrados.");
        }else{

            int precioCantidadProducto;
            int precioTotal = 0;
            ...
            for (int i = 0; i < this.cantActual; i++) {
                ...
                String categoriaProducto = inventario[i].getCategoria();
                int stockProducto = inventario[i].getStock();
                precioCantidadProducto = precioProducto * stockProducto;
                precioTotal += precioCantidadProducto;
                ...
                StdOut.println("Precio del producto: "+precioProducto);
                StdOut.println("Categoria del producto: "+categoriaProducto);
                StdOut.println("Monto acumulado: "+precioCantidadProducto);
                StdOut.println();
            }
            StdOut.println("Precio total del consumo: "+precioTotal);
            StdOut.println("...........................................");
        }
    }
}
